...
public class UseCouponServiceImpl extends BaseOrderService implements UseCouponService {
	...
	public CommonResult<UseCouponResponse> useCoupon(UseCouponRequest request) {
		...
		// 校验优惠券是否正常
		CommonResult<List<CouponQueryResponse>> result = couponMasterFeign.findCoupon(new CouponQueryRequest().setCouponIds(request.getCouponIds()).setOrderId(request.getOrderId()));
		...
		if (shouldPay.intValue() <= 0) {
			throw new CustomException(OrderErrorCodeEnum.EXCEED_PAYMENT_MONEY.getMessage());
		}
		// 更新订单应付金额
